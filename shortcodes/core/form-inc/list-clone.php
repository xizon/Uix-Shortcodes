<?php
class UixShortcodesForm_ListClone {
	...
	public static function add( $args, $_output ) {
        ...
        if ( $type == 'list' ) {
            ...
            if ( is_array( $default ) && !empty( $default ) ) {
                ...
                foreach ( $default[ 'clone_class' ] as $tid_value ) {
					...
					if ( $tid_value[ 'type' ] == 'image' ) {
						...
						$media_js .= '$( document ).uix_uploadMediaCustom( { btnID: "#"+j_'.$id.'+"-trigger_id_'.$loop_trigger_id.'", closebtnID: "#"+j_'.$id.'+"-drop_trigger_id_'.$loop_trigger_id.'" } );'."\n";
					}	
					...
					if ( $tid_value[ 'type' ] == 'color' ) {
						...
						$media_js .= '$( document ).uix_radioSelector( { containerID: "#"+j_'.$id.'+"-trigger_id_'.$loop_trigger_id.'", targetID: "#"+j_'.$id.'+"-'.$loop_trigger_id.'" } );'."\n";
					}		
					
					//-----
					if ( $tid_value[ 'type' ] == 'toggle' && !UixShortcodes::is_IE() ) {
						...
						$media_js .= '$( document ).uix_divToggle( { btnID: "#"+j_'.$id.'+"-trigger_id_'.$loop_trigger_id.'", targetID: '.$toggle_target_id.', list: 1 } );'."\n";
					}						
            ...
            $jscode = '
                ...
                $( document ).uix_dynamicAddinginput({
                    btnID: ".addrow-'.$id.'",
