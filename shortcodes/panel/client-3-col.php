...
( function($) {
	...
	$( document ).ready( function() {
		...
		/* List Item ( step 1) */
		var dynamic_append_box_content = '<?php echo UixShortcodes::dynamic_form_code( 'dynamic-row-uix_sc_client_col3_listitem_logo', $form_html ).UixShortcodes::dynamic_form_code( 'dynamic-row-uix_sc_client_col3_listitem_intro', $form_html ); ?>';
			
		
		
		 /* Callback before custom javascript of sweetalert */
		<?php echo UixShortcodes::sweetalert_before( $form_js, $form_html, $form_js_vars, $form_id, __( 'Insert Client (3 Column)', 'uix-shortcodes' ) ); ?>
				...
				<?php echo UixShortcodes::send_to_editor_before( $form_id ); ?> "[uix_client]"+show_list_item+"<br>[/uix_client]" <?php echo UixShortcodes::send_to_editor_after(); ?>
