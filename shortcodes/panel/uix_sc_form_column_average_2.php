...
if ( $sid == -1 && is_admin() ) {
	...
	if( $currentScreen->base === "post" || $currentScreen->base === "widgets" || $currentScreen->base === "customize" || self::inc_str( $currentScreen->base, '_page_' ) ) {
		...
		'use strict';
			$( function() {  
				<?php echo UixSCFormCore::uixscform_callback( $form_js, $form_id, __( 'Column One Half', 'uix-shortcodes' ) ); ?>					
				...
				/*--**************** Custom shortcode begin ****************-- */
					...
					code = "[uix_column_wrapper top='"+uix_sc_col_average_2_padding_top+"' bottom='"+uix_sc_col_average_2_padding_bottom+"' left='"+uix_sc_col_average_2_padding_left+"' right='"+uix_sc_col_average_2_padding_right+"']" + _vhtml + "[/uix_column_wrapper]";
