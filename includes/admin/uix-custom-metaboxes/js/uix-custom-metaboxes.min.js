...
UixCustomMetaboxesInit.toggleSelect();UixCustomMetaboxesInit.editor(false);UixCustomMetaboxesInit.upload();});}
...
x--;}});jQuery('.uix-cmb__custom-attributes-field').sortable({items:'.uix-cmb__text--div--toggle--sortable',handle:'.uix-cmb__custom-attributes-field__sortablebtn',cursor:'move',scrollSensitivity:40,forcePlaceholderSize:true,forceHelperSize:false,helper:'clone',opacity:0.65,placeholder:'wc-metabox-sortable-placeholder',start:function(event,ui){ui.item.css('background-color','#f6f6f6');},stop:function(event,ui){ui.item.removeAttr('style');UixCustomMetaboxesInit.editor(true);},update:function(event,ui){}});});});return this;},};return new UixCustomMetaboxesConstructor(obj);};var UixCustomMetaboxesInit=new UixCustomMetaboxes();UixCustomMetaboxesInit.getInstance();(function($){"use strict";$(function(){if(typeof(tinymce)!=='undefined'){tinymce.PluginManager.add('customCode',function(editor){function showDialog(){var win=editor.windowManager.open({title:"Source code",body:{type:'textbox',name:'customCode',multiline:true,minWidth:editor.getParam("code_dialog_width",600),minHeight:editor.getParam("code_dialog_height",Math.min(tinymce.DOM.getViewPort().h-200,500)),spellcheck:false,style:'direction: ltr; text-align: left'},onSubmit:function(e){editor.focus();if(editor.readonly!=true){editor.undoManager.transact(function(){editor.setContent(e.data.customCode);});}
