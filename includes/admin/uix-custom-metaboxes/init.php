...
//
		...
		public static function backstage_scripts() {
				 ...
				 ) 
			  {
					...
					wp_enqueue_script( 'wp-color-picker-alpha', UixShortcodes::plug_directory() .'includes/admin/uix-custom-metaboxes/js/' . $wp_color_picker_alpha_uri_name, array( 'wp-color-picker', 'uix-custom-metaboxes' ), '2.1.2', true );
		...
		*/
		public static function addfield_image( $id, $title, $desc, $default, $options = '', $placeholder = '', $desc_primary = '', $enable_table = false ) {
            ...
            $label_controller_up_remove   = esc_attr__( 'Remove', 'uix-shortcodes' );
						...
						if ( is_array ( $options )  && isset( $options[ 'value' ] ) ) {
							...
							foreach ( $options[ 'value' ] as $key => $value  ) {
								...
								<?php if ( $radio_type == 'switch' ) { ?>
									<label data-value="<?php echo esc_attr( $key ); ?>" data-switch-ids="<?php echo esc_attr( $toggle_switch_id ); ?>" class="<?php if ( $br ) { echo 'uix-cmb__label'; } else { echo ''; }; ?> uix-cmb__radio-text uix-cmb__switch-radios <?php if ( $default == esc_attr( $key ) || empty( $default ) ) { echo 'active'; } else { echo ''; }; ?>"><input type="radio" name="<?php echo esc_attr( $id ); ?>_r" value="<?php echo esc_attr( $key ); ?>" <?php echo esc_html( $checked ); ?>/><?php echo UixShortcodes::kses( $value ); ?></label>
								<?php } ?>	
