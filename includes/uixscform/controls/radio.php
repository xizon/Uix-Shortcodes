<?php
class UixSCFormType_Radio extends UixSCFormCore {
			...
			if ( is_array( $default ) && !empty( $default ) ) {
				...
				if ( is_array( $toggle ) && !empty( $toggle ) ) {
					...
					foreach ( $default as $select_key => $select_value ) {
						...
						$optionlist .= '<span data-value="'.$select_key.'" class="'.$selected.' '.( !empty( $toggle_trigger_id ) ? 'uixscform_btn_trigger-toggleswitch_radio' : '' ).'" '.( !empty( $toggle_trigger_id ) ? 'data-targetid="'.$target_id.'"' : '' ).'>'.$select_value.'</span>'.PHP_EOL;
						$optionloop ++;
					}	
