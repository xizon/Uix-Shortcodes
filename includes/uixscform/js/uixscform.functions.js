...
function uixscform_insertCodes( code, conid ) {
	if ( conid == 'content' ) {
		...
		jQuery( '#' + conid ).val( jQuery( '#' + conid ).val() + uixscform_insertToTextarea( code ) );

		//Dynamic listening for the latest value
		jQuery( '#' + conid ).focus().trigger( 'change' );
	}
...
/*! 
 * ************************************
 * Insert value to block(gutenberg)
 *************************************
 */	
...
function uixscform_curModalID() {
	var result = '';
	jQuery( '.uixscform-modal-box' ).each( function()  {
		...
		tinymce.init({
			...
			setup:function( ed ) {
			   ...
			   ed.on( 'change', function(e) {
				...
				ed.addButton('uixsc_highlightcode', {
					...
					tooltip: uix_shortcodes_wp_plugin.lang_mce_hcode_title,
					onclick: function (e) {


						ed.windowManager.open( {
							title: uix_shortcodes_wp_plugin.lang_mce_hcode_title,
							body: [
								{
									type   : 'listbox',
									name   : 'lang',
									label  : uix_shortcodes_wp_plugin.lang_mce_hcode_field_label,
									values : [
										{ value: 'xml', text: 'xml' },
										{ value: 'css', text: 'css' },
...
function uixscform_module_form_ids( tempID, sectionID, colID, returnTempID ) {
	...
	if( typeof tempID !== typeof undefined ) {
		...
		var //Returns the keys of the module template.
			keys                        = uixscform_module_form_keys( jQuery( '#module_tmpl__' + tempID ).html() ),
			...
			all_field_ids              = jQuery( '#uixscform-form-all-field-ids-' + tempID ).attr( 'data-field-ids' ),
			all_clonefield_ids         = '';
