...
editor.addCommand("mceCustomCodeEditor",showDialog);editor.addButton('customCode',{icon:'code',tooltip:'Source code',onclick:showDialog,classes:'customCode'});editor.addMenuItem('customCode',{icon:'code',text:'Source code',context:'tools',onclick:showDialog});});}});})(jQuery);function uixscform_editorInit(id){if(typeof id!==typeof undefined){var vid=id.toString().replace('-editor',''),$sync=jQuery('textarea#'+vid),editorH=$sync.data('editor-height');tinyMCE.execCommand('mceRemoveEditor',true,id);tinymce.init({relative_urls:false,content_css:'',convert_urls:false,media_live_embeds:true,language_url:(uix_shortcodes_wp_plugin.site_lang!='en_US')?uix_shortcodes_wp_plugin.url+'js/tinymce/langs/'+uix_shortcodes_wp_plugin.site_lang+'.js':false,selector:'textarea#'+id,height:editorH,menubar:false,plugins:'textcolor image media hr customCode colorpicker lists fullscreen',toolbar:'removeformat formatselect fontselect forecolor backcolor bold italic underline strikethrough bullist numlist blockquote alignleft aligncenter alignright | uixsc_link uixsc_unlink outdent indent superscript subscript hr uixsc_image uixsc_highlightcode media customCode',setup:function(ed){$sync.addClass('mce-sync');ed.on('change',function(e){var newvalue=ed.getContent().replace(/\r?\n/gm,'').replace(/\.\.\/wp-content\/uploads\//g,uix_shortcodes_wp_plugin.upload_dir_url);$sync.val(newvalue).trigger('change');});function uixsc_mce_insertImage(){var upload_frame;if(upload_frame){upload_frame.open();return;}
