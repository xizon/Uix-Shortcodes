...
return newstr;};function uixscform_rangeSlider(sliderid,outputid,cusunits){var x=document.getElementById(sliderid).value;document.getElementById(outputid).innerHTML=x+cusunits;};function uixscform_insertCodes(code,conid){if(conid=='content'){wp.media.editor.insert(code);}else{jQuery('#'+conid).val(jQuery('#'+conid).val()+uixscform_insertToTextarea(code));jQuery('#'+conid).focus().trigger('change');}};function uixscform_insertToTextarea(s){return(typeof s!="string")?s:s.replace(/<br\s*[\/]?>/gi,'\n');};function uixscform_insertToBlockTextarea(s){return(typeof s!="string")?s:s.replace(/<br\w*\/*>/g,"").replace(/<s>/g,"[s]").replace(/<\/s>/g,"[\/s]").replace(/<strong>/g,"[strong]").replace(/<\/strong>/g,"[\/strong]").replace(/<b>/g,"[b]").replace(/<\/b>/g,"[\/b]").replace(/<em>/g,"[em]").replace(/<\/em>/g,"[\/em]").replace(/<i>/g,"[i]").replace(/<\/i>/g,"[\/i]").replace(/<u>/g,"[u]").replace(/<\/u>/g,"[\/u]").replace(/<del>/g,"[del]").replace(/<\/del>/g,"[\/del]").replace(/<blockquote>/g,"[blockquote]").replace(/<\/blockquote>/g,"[\/blockquote]").replace(/<p>/g,"[p]").replace(/<\/p>/g,"[\/p]");};function uixscform_format_text_decode(str){var newstr='';if(typeof(str)=='string'&&str.length>0){newstr=str.replace(/&quot;/g,'"').replace(/&#39;/g,"'").replace(/&lt;/g,'<').replace(/&gt;/g,'>').replace(/\&amp;\#60;/g,'&lt;').replace(/\&amp;\#62;/g,"&gt;");var re=/\<pre.*?\>([^\>]+)\<\/pre\>/ig;if(re.test(newstr)){newstr=newstr.replace(re,function(m){return m.replace(/\&amp;nbsp;/g,"&nbsp;").replace(/\&amp;\#/g,"&#").replace(/\&\#91;br\&\#93;/g,"<br>");});}}
return newstr;}
...
return hasShortcode;}
function uixscform_curModalID(){var result='';jQuery('.uixscform-modal-box').each(function(){if(jQuery(this).css('display')!='none'){result=jQuery(this).attr('id')
return false;}});return result;};function uixscform_format_slug(str){if(typeof(str)=='string'&&str.length>0){return str.toString().replace(/[^\w\s\-！￥【】\u4e00-\u9eff]/gi,'').replace(/\s/g,'-').replace(/(\-){2,}/g,'-').replace(/\-\s*$/,'').toLowerCase();}else{return'';}}
...
(function($){"use strict";$(function(){if(typeof(tinymce)!=='undefined'){tinymce.PluginManager.add('uixsc_customcode',function(editor){function showDialog(){var win=editor.windowManager.open({title:uix_shortcodes_wp_plugin.lang_mce_sourcecode_title,body:{type:'textbox',name:'uixsc_customcode',multiline:true,minWidth:editor.getParam("code_dialog_width",600),minHeight:editor.getParam("code_dialog_height",Math.min(tinymce.DOM.getViewPort().h-200,500)),spellcheck:false,style:'direction: ltr; text-align: left'},onSubmit:function(e){editor.focus();if(editor.readonly!=true){editor.undoManager.transact(function(){editor.setContent(e.data.uixsc_customcode);});}
editor.selection.setCursorLocation();editor.nodeChanged();}});win.find('#uixsc_customcode').value(editor.getContent({source_view:true}));if(editor.readonly){var id=win.find('#uixsc_customcode')[0]._id;$(win.find('#uixsc_customcode')[0]._elmCache[id]).prop('readonly',true);}}
editor.addCommand("mceCustomCodeEditor",showDialog);editor.addButton('uixsc_customcode',{icon:'code',tooltip:uix_shortcodes_wp_plugin.lang_mce_sourcecode_title,onclick:showDialog,classes:'uixsc_customcode'});editor.addMenuItem('uixsc_customcode',{icon:'code',text:uix_shortcodes_wp_plugin.lang_mce_sourcecode_title,context:'tools',onclick:showDialog});});}});})(jQuery);function uixscform_editorInit(id){if(typeof id!==typeof undefined){var vid=id.toString().replace('-editor',''),$sync=jQuery('textarea#'+vid),editorH=$sync.data('editor-height');tinyMCE.execCommand('mceRemoveEditor',true,id);tinymce.init({relative_urls:false,content_css:'',convert_urls:false,media_live_embeds:true,language_url:(uix_shortcodes_wp_plugin.site_lang!='en_US')?uix_shortcodes_wp_plugin.url+'js/tinymce/langs/'+uix_shortcodes_wp_plugin.site_lang+'.js':false,selector:'textarea#'+id,height:editorH,menubar:false,plugins:'textcolor image media hr uixsc_customcode colorpicker lists fullscreen',toolbar:'removeformat formatselect fontselect forecolor backcolor bold italic underline strikethrough bullist numlist blockquote alignleft aligncenter alignright | uixsc_link uixsc_unlink outdent indent superscript subscript hr uixsc_image uixsc_highlightcode media uixsc_customcode',setup:function(ed){$sync.addClass('mce-sync');ed.on('change',function(e){var newvalue=ed.getContent().replace(/\r?\n/gm,'').replace(/\.\.\/wp-content\/uploads\//g,uix_shortcodes_wp_plugin.upload_dir_url);$sync.val(newvalue).trigger('change');});function uixsc_mce_insertImage(){var upload_frame;if(upload_frame){upload_frame.open();return;}
upload_frame=wp.media({title:uix_shortcodes_wp_plugin.lang_media_title,button:{text:uix_shortcodes_wp_plugin.lang_media_text,},multiple:false});upload_frame.on('select',function(){var attachment;attachment=upload_frame.state().get('selection').first().toJSON();var videoReg=/^.*\.(avi|AVI|wmv|WMV|flv|FLV|mpg|MPG|mp4|MP4)$/gi;if(videoReg.test(attachment.url)){ed.insertContent('<video width="400" height="300" src="'+attachment.url+'" controls></video>');}else{ed.insertContent('<img src="'+attachment.url+'" alt="">');}});upload_frame.open();}
...
return result;}
function uixscform_module_form_ids(tempID,sectionID,colID,returnTempID){var result=[];if(typeof tempID!==typeof undefined){result.push({colID:colID});result.push({sectionID:sectionID});result.push({formID:tempID});var
keys=uixscform_module_form_keys(jQuery('#module_tmpl__'+tempID).html()),cloneTempListKey=[],cloneBeforeTempListIndex=[],cloneBeforeTempListElements=[],cloneTempListElements=[],cloneTempListElementsNew=[],cloneTempListMax=jQuery(jQuery('#module_tmpl__'+tempID).html()).find('[data-clone-max]').attr('data-clone-max'),renderTempID='',renderTempElement1='',renderTempElement2='',all_field_ids=jQuery('#uixscform-form-all-field-ids-'+tempID).attr('data-field-ids'),all_clonefield_ids='';if(typeof all_field_ids!==typeof undefined){var new_all_field_ids_arr=all_field_ids.split(',');for(var i=0;i<new_all_field_ids_arr.length;i++){if(new_all_field_ids_arr[i]!=''&&new_all_field_ids_arr[i].indexOf('_triggerclonelist')<0){var _new_field_name=new_all_field_ids_arr[i],_new_key_id=uixscform_filter_control_id(_new_field_name)+'__fieldID';all_clonefield_ids+=_new_key_id+',';}}}
...
if(returnTempID){result=renderTempID.replace(/\[/g,'').replace(/\]/g,'');}}
return result;}